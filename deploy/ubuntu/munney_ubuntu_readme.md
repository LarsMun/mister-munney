...
└── Production: https://munney.home.munne.me
    ...
    └── Network: proxy + munney-prod-internal
```
...
- ✅ Create project directories (`/srv/munney-dev` and `/srv/munney-prod`)
- ✅ Generate SSH keys for GitHub
- ✅ Clone repositories (develop → dev, main → prod)
...
# Production (⚠️ CAREFUL - this deletes data!)
cd /srv/munney-prod
...
- 🔒 All passwords are auto-generated and stored in `.env` files
- 🔒 `.env` files are NOT in git (listed in `.gitignore`)
- 🔒 Backend volumes are read-only in production
- 🔒 HTTPS enforced via Traefik with Let's Encrypt
- 🔒 Security headers added via Traefik labels
- 🔒 Database ports exposed only for external tools (can be removed)
...
│   ├── .git/                   # Git repository (develop branch)
│   ├── backend/
│   │   ├── .env                # Symfony config
│   │   ├── src/
│   │   └── ...
