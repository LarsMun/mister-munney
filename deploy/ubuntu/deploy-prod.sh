...
echo -e "${YELLOW}⚠️  URL: https://munney.home.munne.me${NC}"
...
if docker ps | grep -q munney-mysql-prod; then
    docker exec munney-mysql-prod mysqldump -u root -p${MYSQL_ROOT_PASSWORD_PROD} money_db_prod > $BACKUP_FILE 2>/dev/null || true
...
docker compose -f docker-compose.yml -f deploy/ubuntu/docker-compose.prod.yml down || true
...
echo "   https://munney.home.munne.me"
echo ""
echo -e "${BLUE}📝 View logs:${NC}"
echo "   docker logs munney-backend-prod -f"
echo "   docker logs munney-frontend-prod -f"
echo "   docker logs munney-mysql-prod -f"
echo ""
echo -e "${BLUE}💾 Database backup:${NC}"
echo "   $BACKUP_FILE"
echo ""
echo -e "${BLUE}🔧 Useful commands:${NC}"
echo "   docker exec -it munney-backend-prod bash"
echo "   docker exec -it munney-mysql-prod mysql -u money -p money_db_prod"
echo ""